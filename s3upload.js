...
S3Upload.prototype.uploadToS3 = function (file) {
  ...
  return Evaporate.create(evaporateOptions).then(
    function (evaporate) {
      var addConfig = {
        name: this.s3Path + this.scrubFilename(file.name),
        file: file,
        xAmzHeadersCommon: this.xAmzHeadersCommon,
        xAmzHeadersAtInitiate: this.xAmzHeadersAtInitiate,
        progress: function (progressValue, stats) {
          return this.onProgress(
            progressValue,
            progressValue === 100 ? "Finalizing" : "Uploading",
            file,
            stats
          );
        }.bind(this),
  ...
  if (filename !== undefined) {
    return (
      ...
      this.evaporate.cancel(
        this.evaporateOptions.bucket +
          "/" +
          this.s3Path +
          this.scrubFilename(filename)
      )
    );
  } else {
